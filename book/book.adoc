...
downloading and working with this code, see Section link:#code[1.2].
...
package. See Section link:#code[1.2] for information about installing
matplotlib.
...
code, see Section link:#code[1.2].
...
In Section link:#cleaning[2.6] we transformed agepreg from centiyears to
...
Figure link:#first_wgt_lb_hist[[first_wgt_lb_hist]] shows the result.
...
Figure link:#first_wgt_oz_hist[[first_wgt_oz_hist]] shows the histogram
...
Figure link:#first_agepreg_hist[[first_agepreg_hist]] shows the
...
Figure link:#first_prglngth_hist[[first_prglngth_hist]] shows the
...
Figure link:#first_nsfg_hist[[first_nsfg_hist]] shows the result.
...
come back to this in Chapter link:#estimation[9].
...
Figure link:#probability_nsfg_pmf[[probability_nsfg_pmf]] shows PMFs of
...
Figure link:#probability_nsfg_pmf[[probability_nsfg_pmf]]:
...
Figure link:#probability_nsfg_diffs[[probability_nsfg_diffs]] shows the
...
Figure link:#class_size1[[class_size1]] shows the result. In the biased
...
In Section link:#dataframe[2.4] we read a pandas DataFrame and used it
...
In Section link:#mean[3.7] we computed the mean of a sample by adding up
...
at birth in pounds. Figure link:#nsfg_birthwgt_pmf[[nsfg_birthwgt_pmf]]
...
Figure link:#example_cdf[[example_cdf]] is a graphical representation of
...
Figure link:#cumulative_prglngth_cdf[[cumulative_prglngth_cdf]] shows
...
Figure link:#cumulative_birthwgt_cdf[[cumulative_birthwgt_cdf]] shows
the result. Compared to
Figure link:#nsfg_birthwgt_pmf[[nsfg_birthwgt_pmf]], this figure makes
...
Figure link:#cumulative_random[[cumulative_random]] shows the result.
...
distribution. Figure link:#analytic_expo_cdf[[analytic_expo_cdf]] shows
...
Figure link:#analytic_interarrival_cdf[[analytic_interarrival_cdf]]
...
Figure link:#analytic_gaussian_cdf[[analytic_gaussian_cdf]] shows CDFs
...
in the NSFG.
Figure link:#analytic_birthwgt_model[[analytic_birthwgt_model]] shows
...
Figure link:#analytic_normal_prob_example[[analytic_normal_prob_example]]
...
Figure link:#analytic_birthwgt_normal[[analytic_birthwgt_normal]] shows
...
Figure link:#brfss_weight[[brfss_weight]] (left) shows the distribution
of adult weights on a linear scale with a normal model.
Figure link:#brfss_weight[[brfss_weight]] (right) shows the same
...
Figure link:#brfss_weight_normal[[brfss_weight_normal]] shows normal
...
possible value. Figure link:#analytic_pareto_cdf[[analytic_pareto_cdf]]
...
Figure link:#populations_pareto[[populations_pareto]] shows the CCDF of
...
Figure link:#populations_normal[[populations_normal]] shows the CDF of
...
as we will see in Chapter link:#analysis[15].
...
In the BRFSS (see Section link:#lognormal[[lognormal]]), the
...
For example, in Section link:#exponential[6.1] we plotted the
...
Figure link:#pdf_example[[pdf_example]] shows the result, as well as a
...
Figure link:#pdf_example[[pdf_example]] shows the normal density
...
review. Figure link:#dist_framework[[dist_framework]] shows how these
...
Figure link:#density_totalwgt_kde[[density_totalwgt_kde]] shows the
...
Figure link:#density_wtkg2_kde[[density_wtkg2_kde]] shows the result.
...
(see Section link:#lognormal[[lognormal]]).
...
The result, in Figure link:#scatter1[8.1] (left), shows the shape of the
...
Figure link:#scatter1[8.1] (right) shows the result. Jittering reduces
...
Figure link:#scatter2[8.2] (left) shows the result. Overlapping data
...
Figure link:#scatter2[8.2] (right) shows the result. An advantage of a
...
Figure link:#scatter3[8.3] shows the result. Between 140 and 200 cm the
...
Figure link:#corr_examples[[corr_examples]] is from
...
The result is shown in Figure link:#estimation1[9.1]. This distribution
...
Figure link:#hypothesis1[10.1] shows the result. The CDF intersects the
...
deviation. In Section link:#visualization[4.3], we saw some evidence
...
Section link:#visualization[4.3], we saw several apparent differences in
...
In Section link:#testdiff[10.3], we simulated the null hypothesis by
...
from the observed values, as in Section link:#power[10.10].
...
Figure link:#linear1[11.1] shows a scatter plot of birth weight and age
...
Section link:#characterizing[8.2]. Figure link:#linear2[11.2] shows the
...
error, and sampling error. As discussed in Chapter link:#estimation[9],
...
Figure link:#linear3[11.3] shows the 50% and 90% confidence intervals
...
Section link:#corrtest[10.5], when we tested the correlation between
...
Section link:#regest[11.4]. To do that, we assumed that the observed
slope was correct and simulated experiments by resampling.

Figure link:#linear4[11.4] shows the sampling distribution of the slope,
from Section link:#regest[11.4], and the distribution of slopes
...
reasons we will see in Section link:#CLT[15.4].
...
sample, but as I mentioned in Section link:#nsfg[2.2], it is not. The
...
In Section link:#regest[11.4], we saw ResampleRows, which chooses rows
...
In Section link:#birth_weights[5.5] we saw that first babies tend to be
...
than what we saw in Section link:#multiple[12.2], which suggests that
...
Figure link:#timeseries1[13.1] shows the result. One apparent feature in
...
As we saw in Section link:#implementation[12.8], model contains exog and
...
of the fitted values. Figure link:#timeseries2[13.2] shows the results
...
Figure link:#timeseries10[13.3] (left) shows the result. The rolling
...
Figure link:#timeseries10[13.3] (right) shows the EWMA for the same
...
Figure link:#timeseries8[13.4] shows the result. The filled data is
...
Section link:#statsmodels[12.1], also provides functions for time series
...
Figure link:#timeseries9[13.5] (left) shows autocorrelation functions
...
Figure link:#timeseries9[13.5] (right) shows autocorrelation functions
...
The linear regressions we used in Section link:#timeregress[13.3] can be
...
Section link:#regest[11.4]. As always, the goal is to use the actual
...
Section link:#timeregress[13.3], to estimate the slope and intercept of
the observed values.
...
Figure link:#timeseries4[13.6] shows the result. The dark gray region
...
But looking at the moving averages in Figure link:#timeseries10[13.3],
...
Figure link:#timeseries5[13.7] shows the result for the medium quality
...
term, as we did in Section link:#nonlinear[12.3].
...
HypothesisTest from Section link:#hypotest[10.2]. It should take a
...
Figure link:#survival1[14.1] (top) shows the CDF of pregnancy length and
...
Figure link:#survival1[14.1] (top) shows the result. The curve is nearly
...
Figure link:#survival1[14.1] (bottom) shows the hazard function for
...
Figure link:#survival2[14.2] (top) shows the estimated hazard function;
...
Figure link:#survival2[14.2] (bottom) shows the result. The survival
...
Figure link:#survival3[14.3] shows the result along with the survival
...
2006–2010 used in Section link:#replication[10.11]; and the Cycle 5 data
...
Figure link:#survival4[14.4] shows the results. Several patterns are
visible:
...
As we saw in Section link:#hazard[14.2], the HazardFunction contains a
...
Figure link:#survival5[14.5] shows the results; I’ve removed the 50s
...
pregnancy length from Section link:#survival[14.1], we can compute the
...
Figure link:#survival6[14.6] (left) shows the expected remaining
...
Figure link:#survival6[14.6] (right) shows the median remaining time
...
latexmath:[\bar{x}]. In Section link:#gorilla[9.3] we approximated this
...
in SciPy, as we saw in Section link:#normal[6.2]. thinkstats2 provides a
...
Figure link:#normal1[15.1] (top row) shows the results. With n=1, the
...
Figure link:#normal1[15.1] (bottom row) shows similar results for a
...
Figure link:#normal2[15.2] (top row) shows distributions of sums of
...
Figure link:#normal2[15.2] (bottom row) shows distributions of sums of
...
example in Section link:#testdiff[10.3]: testing the apparent difference
...
Section link:#testdiff[10.3], which was 0.17.

=== Correlation test

In Section link:#corrtest[10.5] we used a permutation test for the
...
hypothesis. Figure link:#normal4[15.3] shows this distribution along
with the distribution we generated in Section link:#corrtest[10.5] by
...
From Figure link:#normal4[15.3], we can see that the observed
...
In Section link:#casino2[10.7] we used the chi-squared statistic to test
...
Figure link:#normal5[15.4] shows the analytic result along with the
...
Figure link:#normal5[15.4] to compare the analytic results to the
resampling results.
...
think the approach I presented in Chapter link:#testing[10]—simulating
...
[#log_clt]#[log_clt]# In Section link:#lognormal[[lognormal]], we saw
...
In Section link:#usingCLT[15.6] we used the Central Limit Theorem to
