...
a folder called \verb"ThinkStats2/code" with a file called \verb"nsfg.py".
If you run \verb"nsfg.py", it should read a data file, run some tests, and print a
...
about installing \verb"matplotlib".%
\index{thinkplot}%
\index{matplotlib}
...
\verb"log(1-p)" is always defined.%
\index{random module}
...
%
using Equation \eqref{eq:3}.  And if we divide by $n$, the sample mean,
$Z$, is distributed
%
\[ Z \sim \normal~(\mu, \sigma^2/n) \]
%
using Equation \eqref{eq:1} with $a = 1/n$.
...
subtraction using Equation \eqref{eq:2}:
\index{Normal}
