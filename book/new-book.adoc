...
package. See <<code>> for information about installing
matplotlib.
...
In <<cleaning>> we transformed agepreg from centiyears to
...
come back to this in <<estimation>>.
...
The result is shown in <<estimation1>>. This distribution
...
<<characterizing>>. Figure <<linear2>> shows the
...
In <<regest>>, we saw ResampleRows, which chooses rows
...
As we saw in <<implementation>>, model contains exog and
...
of the fitted values. <<timeseries2>> shows the results
...
HypothesisTest from <<hypotest>>. It should take a
...
in SciPy, as we saw in <<normal>>. thinkstats2 provides a
...
<<normal1>> (top row) shows the results. With n=1, the
...
<<normal5>> to compare the analytic results to the
resampling results.
