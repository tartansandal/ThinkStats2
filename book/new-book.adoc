...
// We define an "empty" attribute above so we can provide visual separation between
// words and adjacent inline macros, e.g., `something{empty}footnote:[...]`.
...
folder called `+ThinkStats2/code+` with a file called nsfg.py. If you
run nsfg.py, it should read a data file, run some tests, and print a
...
downloading and working with this code, see <<code>>.
...
`+ReadStataDct+` takes the name of the dictionary file and returns
`+dct+`, a `+FixedWidthVariables+` object that contains the information
from the dictionary file. `+dct+` provides `+ReadFixedWidth+`, which
reads the data file.
...
The result of the index operator is an `+int64+`; the result of the
...
`+pregordr+`, and we have seen that there are 244 variables in total.
...
estimated using `+mosgest * 4.33+` (months of gestation times the
...
`+agepreg+` contains the mother’s age at the end of the pregnancy. In
the data file, `+agepreg+` is encoded as an integer number of
centiyears. So the first line divides each element of `+agepreg+` by
100, yielding a floating-point value in years.
...
`+inplace+` flag tells `+replace+` to modify the existing Series rather
...
functions handle `+nan+` appropriately. But dealing with missing data
...
`+loc+` provides several ways to select rows and columns from a
DataFrame. In this example, the first expression in brackets is the row
indexer; the second expression selects the column.
...
If you are not familiar with `+defaultdict+`, it is in the Python
`+collections+` module. Using `+d+`, we can look up a respondent and get
the indices of that respondent’s pregnancies.
...
selected the `+values+` attribute, which is a NumPy array.
...
comparing `+pregnum+` for each respondent with the number of records in
the pregnancy file.

You can use `+nsfg.MakePregMap+` to make a dictionary that maps from
each `+caseid+` to a list of indices into the pregnancy DataFrame.
...
`+matplotlib+` package. See <<code>> for information
...
In <<cleaning>> we transformed `+agepreg+` from
centiyears to years, and combined `+birthwgt_lb+` and `+birthwgt_oz+`
...
<<first_wgt_oz_hist>> shows the histogram
...
Hist provides methods `+Largest+` and `+Smallest+`, which take an
...
`+align='left'+` to place corresponding bars on either side of the
value.

With `+width=0.45+`, the total width of the two bars is 0.9, leaving
some space between each pair.

Finally, I adjust the axis to show only data between 27 and 46 weeks.
<<first_nsfg_hist>> shows the result.
...
with latexmath:[n-1] in the denominator, rather than `+n+`. This
statistic is used to estimate the variance in a population using a
sample. We will come back to this in <<estimation>>.
...
especially since the units are weeks{empty}latexmath:[^2], or "`square weeks.`"
...
* *distribution*: The values that appear in a sample and the frequency
of each.
...
a frequency expressed as a fraction of the sample size, `+n+`. To get
from frequencies to probabilities, we divide through by `+n+`, which is
...
`+PrePlot+` takes optional parameters `+rows+` and `+cols+` to make a
grid of figures, in this case one row of two figures. The first figure
(on the left) displays the Pmfs using `+thinkplot.Hist+`, as we have
seen before.

The second call to `+PrePlot+` resets the color generator. Then
`+SubPlot+` switches to the second figure (on the right) and displays
the Pmfs using `+thinkplot.Pmfs+`. I used the `+axis+` option to ensure
that the two figures are on the same axes, which is generally a good
idea if you intend to compare two figures.
...
<<probability_nsfg_diffs>> shows the
...
For each class size, `+x+`, we multiply the probability by `+x+`, the
number of students who observe that class size. The result is a new Pmf
that represents the biased distribution.
...
In <<dataframe>> we read a pandas DataFrame and used it
...
++++
Write functions called `+PmfMean+` and `+PmfVar+` that take a Pmf object
...
* *Probability mass function (PMF)*: a representation of a distribution
as a function that maps from values to probabilities.
* *probability*: A frequency expressed as a fraction of the sample size.
* *normalization*: The process of dividing a frequency by a sample size
...
This implementation of `+Percentile+` is not efficient. A better
...
rank, latexmath:[100 \cdot \mathrm{CDF}(x)].
...
the distribution in half, also known as the *median*. Like the mean, the
...
Cdf also provides `+Sample+`, which takes an integer, `+n+`, and returns
a list of `+n+` values chosen at random from the Cdf.
...
distribution. <<analytic_expo_cdf>> shows
...
`+ReadBabyBoom+` reads the data file and returns a DataFrame with
...
it provides a method, `+cdf+`, that evaluates the standard normal CDF:
...
`+norm.cdf+` takes optional parameters: `+loc+`, which specifies the
mean, and `+scale+`, which specifies the standard deviation.
...
<<analytic_gaussian_cdf>> shows CDFs
...
a straight line with intercept `+mu+` and slope `+sigma+`.
...
`+weights+` is a pandas Series of birth weights; `+mean+` and `+std+`
are the mean and standard deviation.

`+FitLine+` takes a sequence of `+xs+`, an intercept, and a slope; it
returns `+xs+` and `+ys+` that represent a line with the given
parameters, evaluated at the values in `+xs+`.

`+NormalProbability+` returns `+xs+` and `+ys+` that contain values from
...
<<brfss_weight>> (right) shows the same
...
<<brfss_weight_normal>> shows normal
...
possible value. <<analytic_pareto_cdf>>
...
<<populations_pareto>> shows the CCDF of
...
<<populations_normal>> shows the CDF of
...
`+random.random+` can return 0 but not 1, so latexmath:[1 - p] can be 1
but not 0, so `+log(1-p)+` is always defined.
...
For example, in <<exponential>> we plotted the
...
downloaded `+hinc06.xls+`, which is an Excel spreadsheet with
...
find `+hinc.py+`, which reads this file.
...
function.
* *model*: A useful simplification. Analytic distributions are often
good models of more complex empirical distributions.
...
`+thinkstats2+` provides a class called Pdf that represents a
probability density function. Every Pdf object provides the following
methods:

* `+Density+`, which takes a value, `+x+`, and returns the density of
...
* `+MakePmf+`, which evaluates `+Density+` at a discrete set of values
and returns a normalized Pmf that approximates the Pdf.
...
`+Density+` and `+GetLinspace+`. Pdf provides `+Render+` and
`+MakePmf+`.
...
`+Density+` uses `+scipy.stats.norm+`, which is an object that
represents a normal distribution and provides `+cdf+` and `+pdf+`, among
other methods (see <<normal>>).
...
<<pdf_example>> shows the result, as well as a
...
`+Density+` takes a value or sequence, calls `+gaussian_kde.evaluate+`,
...
<<pdf_example>> shows the normal density
...
it is: a dictionary wrapper. Its primary attribute is `+d+`, the
...
including `+__init__+`, `+Values+`, `+Items+` and `+Render+`. It also
provides modifier methods `+Set+`, `+Incr+`, `+Mult+`, and `+Remove+`.
...
`+dict+`, Hist, Pmf or Cdf, a pandas Series, a list of (value,
frequency) pairs, or a sequence of values.
...
`+xs+` is the sorted list of values; `+freqs+` is the list of
corresponding frequencies. `+np.cumsum+` computes the cumulative sum of
...
Cdf is proportional to latexmath:[n \log n].
...
`+np.roll+` shifts the elements of `+a+` to the right, and "`rolls`" the
last one back to the beginning. We replace the first element of `+b+`
with 0 and then compute the difference `+a-b+`. The result is a NumPy
array of probabilities.

Cdf provides `+Shift+` and `+Scale+`, which modify the values in the
Cdf, but the probabilities should be treated as immutable.
...
cm{empty}latexmath:[^2], the third moment is in cm{empty}latexmath:[^3], and so on.
...
<<density_totalwgt_kde>> shows the
...
<<density_wtkg2_kde>> shows the result.
...
information about household income, and converted it to `+hinc06.csv+`,
a CSV file you will find in the repository for this book. You will also
find `+hinc2.py+`, which reads this file and transforms the data.
...
are distributed in each range. `+hinc2.py+` provides
`+InterpolateSample+`, which shows one way to model this data. It takes
a DataFrame with a column, `+income+`, that contains the upper bound of
each range, and `+freq+`, which contains the number of respondents in
each frame.
...
* *Probability density*: A quantity that can be integrated over a range
...
<<scatter1>> (right) shows the result. Jittering reduces
...
<<scatter2>> (left) shows the result. Overlapping data
...
<<scatter2>> (right) shows the result. An advantage of a
...
`+arange+` makes a NumPy array of bins from 135 to, but not including,
...
<<scatter3>> shows the result. Between 140 and 200 cm the
...
provide known means. If `+xs+` and `+ys+` are Python sequences,
`+np.asarray+` converts them to NumPy arrays. If they are already NumPy
arrays, `+np.asarray+` does nothing.
...
that we have not covered yet, and `+np.cov+` returns a covariance
...
relationship between two variables.
* *standardize*: To transform a set of values so that their mean is 0
and their variance is 1.
...
* *treatment group*: A group in a controlled trial that receives some
...
Again, `+n+` is the size of the sample, and `+m+` is the number of times
we play the game. `+means+` is the list of estimates based on
latexmath:[\bar{x}]. `+medians+` is the list of medians.
...
`+estimates+` is a list of estimates; `+actual+` is the actual value
being estimated. In practice, of course, we don’t know `+actual+`; if we
did, we wouldn’t have to estimate it. The purpose of this experiment is
to compare the performance of the two estimators.
...
Again, `+n+` is the sample size and `+m+` is the number of times we play
the game. `+np.var+` computes latexmath:[S^2] by default and
latexmath:[S_{n-1}^2] if you provide the argument `+ddof=1+`, which
...
As `+m+` increases, we expect the mean error for latexmath:[S_{n-1}^2]
to approach 0.
...
`+mu+` and `+sigma+` are the _hypothetical_ values of the parameters.
...
1000 simulations and then compute the distribution, `+cdf+`, of the
estimates. The result is shown in <<estimation1>>. This
distribution is called the *sampling distribution* of the estimator. It
...
Write a function that takes a goal-scoring rate, `+lam+`, in goals per
game, and simulates a game by generating the time between goals until
the total time exceeds 1 game, then returns the number of goals scored.
...
* *confidence interval*: An interval that represents the expected range
...
Bayesian inference{empty}footnote:[For more about Bayesian inference, see the
...
based on `+HypothesisTest+` inherit `+__init__+` and `+PValue+` and
provide `+TestStatistic+`, `+RunModel+`, and optionally `+MakeModel+`.

`+__init__+` takes the data in whatever form is appropriate. It calls
`+MakeModel+`, which builds a representation of the null hypothesis,
then passes the data to `+TestStatistic+`, which computes the size of
the effect in the sample.

`+PValue+` computes the probability of the apparent effect under the
null hypothesis. It takes as a parameter `+iters+`, which is the number
...
As a simple example{empty}footnote:[Adapted from MacKay, _Information Theory,
...
`+data+` is a pair of sequences, one for each group. The test statistic
is the absolute difference in the means.

`+MakeModel+` records the sizes of the groups, `+n+` and `+m+`, and
combines the groups into one NumPy array, `+self.pool+`.

`+RunModel+` simulates the null hypothesis by shuffling the pooled
values and splitting them into two groups with sizes `+n+` and `+m+`. As
always, the return value from `+RunModel+` has the same format as the
observed data.
...
`+MakeFrames+` reads the NSFG data and returns DataFrames representing
all live births, first babies, and others. We extract pregnancy lengths
as NumPy arrays, pass them as data to `+DiffMeansPermute+`, and compute
...
<<hypothesis1>> shows the result. The CDF intersects the
...
called *one-sided* because it only counts one side of the distribution
...
deviation. In <<visualization>>, we saw some evidence
...
value of Pearson’s correlation. `+RunModel+` shuffles the `+xs+` and
...
I use `+dropna+` with the `+subset+` argument to drop rows that are
missing either of the variables we need.
...
drawing random samples from `+values+`. `+RunModel+` uses `+Hist+` to
...
weight to large deviations. Dividing through by `+expected+`
...
<<visualization>>, we saw several apparent differences in
...
use, and `+expected_probs+`, which is the probability of each value in
...
`+TestStatistic+` computes the chi-squared statistic for first babies
and others, and adds them.
...
`+thinkstats2.SampleRows+` to select a random subset of the rows in a
DataFrame.
...
from the observed values, as in <<power>>.
...
* *resampling test*: A way to compute p-values by generating samples,
with replacement, from an observed dataset.
* *two-sided test*: A test that asks, "`What is the chance of an effect
as big as the observed effect, positive or negative?`"
* *one-sided test*: A test that asks, "`What is the chance of an effect
...
not.
* *false negative*: The conclusion that an effect is due to chance when
it is not.
...
Suppose we have a sequence of points, `+ys+`, that we want to express as
a function of another sequence `+xs+`. If there is a linear relationship
between `+xs+` and `+ys+` with intercept `+inter+` and slope `+slope+`,
...
the ones that minimize the residuals.
...
residual, and minimize total cost, `+sum(cost(res))+`. However,
...
`+LeastSquares+` takes sequences `+xs+` and `+ys+` and returns the
estimated parameters `+inter+` and `+slope+`. For details on how it
works, see
...
<<characterizing>>. <<linear2>> shows the
...
The parameters `+slope+` and `+inter+` are estimates based on a sample;
like other estimates, they are vulnerable to sampling bias, measurement
error, and sampling error. As discussed in <<estimation>>,
...
`+ResampleRows+` to resample the observed pregnancies. We’ve already
seen `+SampleRows+`, which chooses random rows from a DataFrame.
...
`+xs+` is the sequence of mother’s age. `+inters+` and `+slopes+` are
the estimated parameters generated by `+SamplingDistributions+`.
`+percent+` indicates which confidence interval to plot.

`+PlotConfidenceIntervals+` generates a fitted line for each pair of
`+inter+` and `+slope+` and stores the results in a sequence,
`+fys_seq+`. Then it uses `+PercentileRows+` to select the upper and
lower percentiles of `+y+` for each value of `+x+`. For a 90% confidence
...
your guess is `+Std(ys)+`, which is 1.41 lbs. So in this example,
knowing a mother’s age does not improve the predictions substantially.
...
`+Var(res)+` is the MSE of your guesses using the model, `+Var(ys)+` is
the MSE without it. So their ratio is the fraction of MSE that remains
if you use the model, and latexmath:[R^2] is the fraction of MSE the
model eliminates.
...
of predictive power. In my opinion, `+Std(res)+` is the best
representation of the quality of prediction, especially if it is
presented in relation to `+Std(ys)+`.
...
<<corrtest>>, when we tested the correlation between
...
<<regest>>. To do that, we assumed that the observed
slope was correct and simulated experiments by resampling.

<<linear4>> shows the sampling distribution of the slope,
from <<regest>>, and the distribution of slopes
...
sample, but as I mentioned in <<nsfg>>, it is not. The
...
the respondent represents. This value is called a *sampling weight*, or
just "`weight.`"
...
In <<regest>>, we saw `+ResampleRows+`, which chooses
...
weights. `+ResampleRowsWeighted+` takes a DataFrame, resamples rows
...
these weights is `+finalwt+`. Use resampling, with and without weights,
...
syntax called `+patsy+`; in this example, the `+~+` operator separates
...
The results are also available as attributes. `+params+` is a Series
...
The p-value associated with `+agepreg+` is `+5.7e-11+`, which is less
than latexmath:[0.001], as expected.

`+results.rsquared+` contains latexmath:[R^2], which is
latexmath:[0.0047]. `+results+` also provides `+f_pvalue+`, which is the
p-value associated with the model as a whole, similar to testing whether
latexmath:[R^2] is statistically significant.

And `+results+` provides `+resid+`, a sequence of residuals, and
`+fittedvalues+`, a sequence of fitted values corresponding to
`+agepreg+`.
...
a simpler function called `+SummarizeResults+`. Here are the results of
this model:
...
`+Std(ys)+` is the standard deviation of the dependent variable, which
is the RMSE if you have to guess birth weights without the benefit of
any explanatory variables. `+Std(res)+` is the standard deviation of the
...
In <<birth_weights>> we saw that first babies tend to be
...
first babies and false otherwise. Then we fit a model using `+isfirst+`
...
value for this model is small, which means that `+isfirst+` doesn’t
...
actually accounted for by `+agepreg+`. And the p-value for `+isfirst+`
is about 2.5%, which is on the border of statistical significance.
...
The quadratic model of `+agepreg+` accounts for more of the variability
in birth weight; the parameter for `+isfirst+` is smaller in this model,
...
mother’s age in the model, the effect of `+isfirst+` gets smaller, and
the remaining effect might be due to chance.
...
The `+join+` method is invoked on `+live+`, which is considered the
"`left`" table, and passed `+resp+`, which is the "`right`" table. The
keyword argument `+on+` indicates the variable used to match up rows
from the two tables.

In this example some column names appear in both tables, so we have to
provide `+rsuffix+`, which is a string that will be appended to the
names of overlapping columns from the right table. For example, both
tables have a column named `+race+` that encodes the race of the
respondent. The result of the join contains two columns named `+race+`
and `+race_r+`.
...
append the results to a list. The models all include `+agepreg+`, since
...
Next is `+race+`, which indicates whether the respondent is white,
...
race acts as a *proxy variable*, so apparent correlations with race are
often caused, at least in part, by other factors.

The next variable on the list is `+nbrnaliv+`, which indicates whether
...
co-worker is expecting twins, that would help.

Next on the list is `+paydu+`, which indicates whether the respondent
...
This formula uses some syntax we have not seen yet: `+C(race)+` tells
...
`+babysex==1+` converts it to boolean, True for male and false for
female.
...
than what we saw in <<multiple>>, which suggests that
...
Notice that when `+log_o+` is greater than 0, `+o+` is greater than 1
and `+p+` is greater than 0.5.

The likelihood of an outcome is `+p+` when `+y==1+` and `+1-p+` when
`+y==0+`. For example, if we think the probability of a boy is 0.8 and
the outcome is a boy, the likelihood is 0.8; if the outcome is a girl,
the likelihood is 0.2. We can compute that like this:
...
`+logit+` takes the same arguments as `+ols+`, a formula in Patsy syntax
and a DataFrame. The result is a Logit object that represents the model.
It contains attributes called `+endog+` and `+exog+` that contain the
*endogenous variable*, another name for the dependent variable, and the
*exogenous variables*, another name for the explanatory variables. Since
...
The result of `+model.fit+` is a BinaryResults object, which is similar
to the RegressionResults object we got from `+ols+`. Here is a summary
of the results:
...
round off to 0 or 1. Multiplying by `+actual+` yields 1 if we predict a
boy and get it right, 0 otherwise. So, `+true_pos+` indicates "`true
positives`".
...
the NSFG dataset, this variable is called `+numbabes+`.
...
encoded in a variable called `+rmarital+`.
...
* *control variable*: A variable included in a regression to eliminate
or "`control for`" a spurious relationship.
...
* *odds*: An alternative way of representing a probability,
latexmath:[p], as the ratio of the probability and its complement,
latexmath:[p / (1-p)].
...
`+groups+`; used in a for loop, it iterates the names of the groups and
the DataFrames that represent them. Since the values of `+quality+` are
`+low+`, `+medium+`, and `+high+`, we get three groups with those names.
...
`+date+` and `+ppg+`. We select these two columns, then group by
`+date+`.
...
result of `+aggregate+` is a DataFrame with one row for each date and
one column, `+ppg+`.

Dates in these DataFrames are stored as NumPy `+datetime64+` objects,
...
`+date+` by copying the `+index+`, then adds `+years+`, which contains
...
`+PrePlot+` with `+rows=3+` means that we are planning to make three
...
As we saw in <<implementation>>, `+model+` contains
...
The first two values are `+nan+`; the next value is the mean of the
...
new DataFrame with all of the data from `+daily+`, but including rows
for all dates, filled with `+nan+`.
...
the trend. The first 29 values are `+nan+`, and wherever there’s a
missing value, it’s followed by another 29 `+nan+`s. There are ways to
fill in these gaps, but they are a minor nuisance.
...
<<timeseries10>> (right) shows the EWMA for the same
...
corresponding value from `+ewma+`. The `+inplace+` flag tells `+fillna+`
...
`+resid+` contains the residual values, not including days when `+ppg+`
is `+nan+`. `+fake_data+` contains the sum of the moving average and a
...
After the shift, the first `+lag+` values are `+nan+`, so I use a slice
to remove them before computing `+Corr+`.
...
<<statsmodels>>, also provides functions for time series
...
The `+unbiased+` flag tells `+acf+` to correct the estimates for the
...
365, we can confirm that `+acf+` and `+SerialCorr+` yield approximately
the same results:
...
`+frisat+` is a boolean Series, `+True+` if the day of the week is
Friday or Saturday. `+fake+` is a new DataFrame, initially a copy of
`+daily+`, which we modify by adding random values to `+ppg+`.
`+frisat.sum()+` is the total number of Fridays and Saturdays, which is
the number of random values we have to generate.

<<timeseries9>> (right) shows autocorrelation functions
...
The linear regressions we used in <<timeregress>> can be
used for prediction. The RegressionResults class provides `+predict+`,
...
`+results+` is a RegressionResults object; `+years+` is the sequence of
time values we want predictions for. The function constructs a
DataFrame, passes it to `+predict+`, and returns the result.
...
<<regest>>. As always, the goal is to use the actual
...
`+daily+` is a DataFrame containing the observed prices; `+iters+` is
the number of simulations to run.

`+SimulateResults+` uses `+RunLinearModel+`, from
<<timeregress>>, to estimate the slope and intercept of
the observed values.
...
<<timeseries4>> shows the result. The dark gray region
...
But looking at the moving averages in <<timeseries10>>,
...
`+RunLinearModel+` that runs that quadratic model, but after that you
...
. To predict values at future times, compute `+inter + slope * dt+`,
where `+dt+` is the difference between the time of the prediction and
the time of the last observation.

Use this method to generate predictions for a year after the last
observation. A few hints:

* Use `+timeseries.FillMissing+` to fill in missing values before
running this analysis. That way the time between consecutive elements is
consistent.
* Use `+Series.diff+` to compute differences between successive
elements.
...
decrease.
* *serial correlation*: Correlation between a time series and a shifted
or lagged version of itself.
...
`+SurvivalFunction+` provides two properties: `+ts+`, which is the
sequence of lifetimes, and `+ss+`, which is the survival curve. In
...
<<survival1>> (top) shows the result. The curve is nearly
...
`+d+` can be a dictionary or any other type that can initialize a
Series, including another Series. `+label+` is a string used to identify
the HazardFunction when plotted.
...
<<survival1>> (bottom) shows the hazard function for
...
The result is stored in a Pandas `+Series+` that maps from each age to
the estimated hazard function at that age.
...
married at that age) and the number of events censored at `+t+` (that
is, the number of women interviewed at `+t+` whose future marriage dates
...
Finally, we pass `+lams+` to the `+HazardFunction+` constructor and
return the result.
...
<<survival2>> (top) shows the estimated hazard function;
...
`+ss+` is the cumulative product of the complementary hazard function,
so it is the survival curve.

Because of the way `+SurvivalFunction+` is implemented, we have to
...
`+iters+`, the number of times to resample. It computes `+ts+`, which is
the sequence of ages where we will evaluate the survival curves.
...
least `+t+` when they were interviewed. So the leftmost part of the
curve includes data from all respondents, but the rightmost part
includes only the oldest respondents.
...
Timestamps, which is converted to a `+DateTimeIndex+`. Finally, we
extract `+year+` and compute decades.
...
As we saw in <<hazard>>, the HazardFunction contains a
Series that maps from latexmath:[t] to latexmath:[\lambda(t)].
`+Extend+` finds `+last+`, which is the last index in `+self.series+`,
selects values from `+other+` that come later than `+last+`, and appends
them onto `+self.series+`.
...
pregnancy length from <<survival>>, we can compute the
...
`+cutoff+` is the highest probability in the Cdf, which is 1 if the Cdf
is complete, and otherwise less than 1. If the Cdf is incomplete, we
plug in the provided value, `+filler+`, to cap it off.
...
`+RemainingLifetime+` takes `+filler+`, which is passed along to
`+MakePmf+`, and `+func+` which is the function used to summarize the
distribution of remaining lifetimes.

`+pmf+` is the Pmf of lifetimes extracted from the SurvivalFunction.
`+d+` is a dictionary that contains the results, a map from current age,
`+t+`, to expected remaining lifetime.
...
<<survival6>> (right) shows the median remaining time
...
latexmath:[\bar{x}]. In <<gorilla>> we approximated this
...
in SciPy, as we saw in <<normal>>. `+thinkstats2+`
...
Given a probability, `+p+`, it returns the corresponding percentile from
a normal distribution with parameters `+mu+` and `+sigma+`. For the 90%
confidence interval of latexmath:[\bar{x}], we compute the 5th and 95th
...
in the previous section. Finally, Normal provides `+Percentile+`, which
...
`+MakeExpoSamples+` generates samples of sums of exponential values (I
use "`exponential values`" as shorthand for "`values from an exponential
distribution`"). `+beta+` is the parameter of the distribution;
`+iters+` is the number of sums to generate.

To explain this function, I’ll start from the inside and work my way
out. Each time we call `+np.random.exponential+`, we get a sequence of
`+n+` exponential values and compute its sum. `+sample+` is a list of
these sums, with length `+iters+`.

It is easy to get `+n+` and `+iters+` confused: `+n+` is the number of
terms in each sum; `+iters+` is the number of sums we compute in order
to characterize the distribution of sums.
...
probability plot is not a straight line. But with `+n=10+` the
distribution of the sum is approximately normal, and with `+n=100+` it
is all but indistinguishable from normal.

<<normal1>> (bottom row) shows similar results for a
...
<<normal2>> (top row) shows distributions of sums of
Pareto values. Even with `+n=100+` the normal probability plot is far
from straight.
...
`+normal+` is a list of correlated normal values. `+uniform+` is a
sequence of uniform values between 0 and 1. `+expo+` is a correlated
sequence of exponential values. `+ppf+` stands for "`percent point
function,`" which is another name for the inverse CDF.

<<normal2>> (bottom row) shows distributions of sums of
correlated exponential values with `+rho=0.9+`. The correlation slows
the rate of convergence; nevertheless, with `+n=100+` the normal
...
example in <<testdiff>>: testing the apparent difference
...
`+mean+` and `+var+` are the mean and variance of `+data+`. We
...
`+Prob+` to compute the probability of a difference as large as
`+delta+` under the null hypothesis:
...
<<testdiff>>, which was 0.17.

=== Correlation test

In <<corrtest>> we used a permutation test for the
...
correlation. `+ps+` contains the corresponding probabilities, computed
using the CDF of the Student’s t-distribution implemented in SciPy. The
parameter of the t-distribution, `+df+`, stands for "`degrees of
...
hypothesis. <<normal4>> shows this distribution along
with the distribution we generated in <<corrtest>> by
...
From <<normal4>>, we can see that the observed
...
In <<casino2>> we used the chi-squared statistic to test
...
coincidence{empty}footnote:[Not really.], it is called the chi-squared
...
<<normal5>> shows the analytic result along with the
...
something like <<normal5>> to compare the analytic
results to the resampling results.
...
think the approach I presented in <<testing>>—simulating
...
In a recent paper{empty}footnote:["`Evidence for the persistent effects of an
